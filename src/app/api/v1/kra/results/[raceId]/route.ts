...
import { fetchRaceResult } from '@/lib/api/kra';
...
export async function GET(
  ...
  try {
    ...
    const result = await fetchRaceResult(meet!, raceNo!, date);
      ...
      return NextResponse.json(
        {
          ...
          error: {
            code: 'NOT_FOUND',
            message: `Race result not found: ${raceId}`,
          },
          timestamp: new Date().toISOString(),
        },
        { status: 404 }
    ...
    console.error('[/api/v1/kra/results/[raceId]] Error:', error);
