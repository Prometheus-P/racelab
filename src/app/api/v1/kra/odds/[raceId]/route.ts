...
import { fetchRaceOdds } from '@/lib/api/kra';
...
export async function GET(
  ...
  try {
    ...
    // Fetch odds for specific race
    const odds = await fetchRaceOdds(meet!, raceNo!, date);
      ...
      return NextResponse.json(
        {
          ...
          error: {
            code: 'NOT_FOUND',
            message: `Odds not found for race: ${raceId}`,
          },
          timestamp: new Date().toISOString(),
        },
        { status: 404 }
    ...
    console.error('[/api/v1/kra/odds/[raceId]] Error:', error);
