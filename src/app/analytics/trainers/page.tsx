 ...
 *
 * 조교사 랭킹 페이지 (실제 KRA API 연동)
 */

'use client';

import React, { useState, useEffect } from 'react';
import { fetchTrainerRanking } from '@/lib/analytics/api';
import type { TrainerStats, RankingFilters, RankingResult } from '@/lib/analytics/types';
...
export default function TrainersPage() {
  ...
  const [result, setResult] = useState<RankingResult<TrainerStats>>({
    items: [],
    total: 0,
    page: 1,
    limit: 20,
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const data = await fetchTrainerRanking(filters);
        setResult(data);
      } catch (err) {
        console.error('[TrainersPage] Error:', err);
        setError('조교사 데이터를 불러오는데 실패했습니다.');
      } finally {
        setLoading(false);
      }
    };
    ...
    <main className="min-h-screen bg-gray-50 py-8">
      <div className="mx-auto max-w-6xl px-4">
        ...
        <div className="mb-6">
          ...
          <h1 className="mt-2 text-3xl font-bold text-gray-900">조교사 랭킹</h1>
          <p className="mt-2 text-gray-600">조교사별 승률, ROI, 관리마 현황을 비교합니다.</p>
        </div>
